...
// --- FIX LINKAGE : On déclare tout en extern "C" pour le linker ---
extern "C" {
    void init_pics();
...
public:
    ...
    void putchar(char c) {
        if (c == '\n') { column = 0; if (++row == VGA::HEIGHT) scroll(); return; }
        const size_t index = row * VGA::WIDTH + column;
        VGA::BUFFER[index] = VGA::make_entry(c, color);
        if (++column == VGA::WIDTH) { column = 0; if (++row == VGA::HEIGHT) scroll(); }
    }
...
// --- FIX : On implémente les fonctions avec le bon linkage ---
...
void draw_gui() {
    ...
    terminal.write_at(20, 0, "MyOS64 - UEFI Ready", VGA::make_color(VGA::YELLOW, VGA::BLUE));
...
void process_command() {
    command_pos = 0;
    terminal.row = 20; terminal.column = 2;
}

extern "C" void kernel_main() {
    draw_gui();
    //init_pics(); // Décommente si tu veux que ça tourne
    //IDT::initialize();
    //Keyboard::initialize();
